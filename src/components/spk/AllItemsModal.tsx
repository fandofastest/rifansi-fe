...
export const AllItemsModal: React.FC<AllItemsModalProps> = ({ open, onClose, items }) => {
  if (!open) return null;
            ...
            <table className="w-full text-sm border border-gray-200 dark:border-white/[0.08] rounded-lg bg-white dark:bg-gray-800">
              <thead>
                <tr className="bg-gray-100 dark:bg-gray-700">
                  ...
                  <th className="p-2 text-left text-black dark:text-white">Volume NR</th>
                  <th className="p-2 text-left text-black dark:text-white">Volume R</th>
                  <th className="p-2 text-left text-black dark:text-white">Harga NR</th>
                  <th className="p-2 text-left text-black dark:text-white">Harga R</th>
                ...
                {items.map((wi, idx) => (
                  <tr key={wi.workItem?.id || idx} className="border-t border-gray-100 dark:border-white/[0.04]">
                    ...
                    <td className="p-2 text-black dark:text-white">
                      {wi.boqVolume?.nr?.toLocaleString('id-ID') || '-'}
                    </td>
                    <td className="p-2 text-black dark:text-white">
                      {wi.boqVolume?.r?.toLocaleString('id-ID') || '-'}
                    </td>
                    <td className="p-2 text-black dark:text-white">
                      {wi.rates?.nr?.rate?.toLocaleString('id-ID', { style: 'currency', currency: 'IDR' }) || '-'}
                    </td>
                    <td className="p-2 text-black dark:text-white">
                      {wi.rates?.r?.rate?.toLocaleString('id-ID', { style: 'currency', currency: 'IDR' }) || '-'}
                    </td>
                    <td className="p-2 text-black dark:text-white">
