...
export default function SalaryComponentManagementPage() {
  ...
  const getAvailableRoles = () => {
    // Filter out roles that already have salary components
    const assignedRoleIds = salaryComponents.map(comp => comp.personnelRole.id);
    return personnelRoles.filter(role => !assignedRoleIds.includes(role.id));
  };
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        ...
        <div className="relative">
          <select
            ...
            value=""
          >
            <option value="" className="text-black dark:text-white dark:bg-gray-900">Tambah Komponen Gaji Baru</option>
          ...
          <table className="w-full table-auto border-collapse bg-white dark:bg-gray-900 dark:text-bodydark">
            ...
            <tbody className="dark:bg-gray-900">
              {salaryComponents.map((component) => {
                const details = salaryDetailsMap[component.id];
                return (
                <tr key={component.id} className="border-b border-[#eee] dark:border-gray-700 dark:bg-gray-900 hover:bg-gray-50 dark:hover:bg-gray-800">
                  <td className="border-b border-[#eee] py-5 px-4 pl-9 dark:border-gray-700 xl:pl-11">
                    <h5 className="font-medium text-black dark:text-white">
                      {component.personnelRole.roleName}
                    </h5>
                    <p className="text-sm text-gray-600 dark:text-gray-400">{component.personnelRole.roleCode}</p>
                  </td>
