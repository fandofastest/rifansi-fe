...
import { createOvertimeRate } from "@/services/overtimeRate";
import { toast } from "react-hot-toast";

interface AddOvertimeRateModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

export default function AddOvertimeRateModal({ 
  isOpen, 
  onClose, 
  onSuccess
}: AddOvertimeRateModalProps) {
  const { token } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await createOvertimeRate(formData, token);
      toast.success("Tarif lembur berhasil dibuat");
      onSuccess();
      onClose();
    } catch (err) {
      console.error("Error creating overtime rate:", err);
      setError("Gagal membuat tarif lembur");
      toast.error("Gagal membuat tarif lembur");
    } finally {
      setLoading(false);
    }
  };
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex items-center justify-end gap-3">
          ...
          <Button variant="primary" disabled={loading}>
            {loading ? "Menyimpan..." : "Simpan Tarif Lembur"}
          </Button>
        </div>
      </form>
    </Modal>
