...
import { getOvertimeRates, deleteOvertimeRate, OvertimeRate } from "@/services/overtimeRate";
import Button from "@/components/ui/button/Button";
import toast from "react-hot-toast";
import AddOvertimeRateModal from "./AddOvertimeRateModal";
import EditOvertimeRateModal from "./EditOvertimeRateModal";

export default function OvertimeRateManagementPage() {
  const { token } = useAuth();
  ...
  const [selectedOvertimeRateId, setSelectedOvertimeRateId] = useState<string | null>(null);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const rates = await getOvertimeRates(token);
      // Sort by waktuKerja ascending
      rates.sort((a, b) => a.waktuKerja - b.waktuKerja);
      setOvertimeRates(rates);
    } catch (error) {
      console.error("Error fetching data:", error);
      toast.error("Gagal memuat data tarif lembur");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleEdit = (overtimeRateId: string) => {
    setSelectedOvertimeRateId(overtimeRateId);
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm("Apakah Anda yakin ingin menghapus tarif lembur ini?")) {
      try {
        if (!token) {
          throw new Error("Token is missing");
        }
        const success = await deleteOvertimeRate(id, token);
        if (success) {
          toast.success("Tarif lembur berhasil dihapus");
          fetchData();
        } else {
          toast.error("Gagal menghapus tarif lembur");
        }
      } catch (error) {
  ...
  return (
    <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-gray-700 dark:bg-gray-900 sm:px-7.5 xl:pb-1">
      ...
      {overtimeRates.length === 0 ? (
        <div className="flex justify-center items-center h-40 bg-white dark:bg-gray-900">
          <p className="text-gray-500 dark:text-gray-400">
            Belum ada tarif lembur. Silakan tambahkan tarif lembur dengan mengklik tombol di atas.
          </p>
        </div>
      ) : (
          ...
          <table className="w-full table-auto border-collapse bg-white dark:bg-gray-900 dark:text-bodydark">
            ...
            <tbody className="dark:bg-gray-900">
              {overtimeRates.map((rate) => (
                <tr key={rate.id} className="border-b border-[#eee] dark:border-gray-700 dark:bg-gray-900 hover:bg-gray-50 dark:hover:bg-gray-800">
      ...
      {editModalOpen && selectedOvertimeRateId && (
        <EditOvertimeRateModal
          isOpen={editModalOpen}
          onClose={() => setEditModalOpen(false)}
          onSuccess={handleEditSuccess}
          overtimeRateId={selectedOvertimeRateId}
        />
      )}
    </div>
  );
